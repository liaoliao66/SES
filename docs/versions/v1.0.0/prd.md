...
version: v1.0.0
updated: 2026-07-15
iteration: 隐患库单选+门户入口；规范重写；异常默认空行+整改责任人或签+查看加强+已复核可见范围业务约定
---
...
| **版本号** | v1.0.0 |
| **更新日期** | 2026-07-15 |
| **迭代说明** | ①隐患库**单选**添加；②PC 门户隐患公示操作入口；③规范重写（仅企微 H5 + PC，剥离平台公共能力 UI）。本轮补充：新增检查记录**默认异常**并自动带出 **1 条可编辑空白隐患行**（仍可再从隐患库添加）；基础字段去掉「责任人」，仅保留异常侧「整改责任人」；异常提交后触发整改流程（**多人=或签**）；整改通知列表/详情强化「查看」；已复核可见范围仅写业务约定，权限在平台配置、不画权限页。 |
...
| **流程审批** | 审批发起、节点、抄送、驳回、审批记录办理页 — 本产品不设计；业务仅约定状态结果与数据回写（如作业票签名时间）；整改多人责任人约定为**或签**，节点由平台配置 |
| **角色/菜单/按钮/数据权限配置** | 权限配置后台 — 本产品不设计、不画原型页；业务侧仅描述「谁在业务上应看到哪些数据」（含已复核整改可见范围），实施时在平台配置 |
...
flowchart TD
  ...
  Check --> Result{检查结果 默认异常}
  Result -->|正常| ArchiveOK[保存归档]
  Result -->|异常| Haz[自动带出1条可编辑空白隐患行 可再从隐患库单选添加]
  Haz --> SubmitChk[提交检查记录]
  SubmitChk --> AutoRect[自动创建整改通知并触发整改流程]
  AutoRect --> FillRect[整改责任人或签任一填写并通过即可完结]
...
- **正常流程**：三大业务域各自完成「录入 → 流转 → 闭环」；检查记录默认异常，自动带出 1 条可编辑空白隐患行并可继续从隐患库添加；异常提交后自动建整改通知并触发整改流程（整改责任人多人时为或签）；门户提供隐患查看/处理入口。
...
- **正常流程**：新增默认「异常」；系统先自动带出 **1 条可编辑空白隐患行**（空白表单可直接填写），仍可通过「添加」从隐患库**单选**继续追加；填写整改责任人（可多人）、截止日期、风险等级后提交；系统自动创建整改通知并触发整改流程。多名整改责任人为**或签**：任一人填写并通过即可完结本通知。隐患项全部完成后进入待复核/已复核。
- **边界情况**：从隐患库添加时未选点确定 → 提示「请选择一条隐患」；重复添加同一隐患库条目 → 提示已添加；空白行未填完整不可提交；隐患未全部完成前不可进入「待复核」。
- **异常兜底**：隐患库无结果展示「暂无数据」；自动建单失败提示可重试。整改流程办理、或签节点由平台支撑，业务侧不提供审批操作页。
...
sequenceDiagram
  ...
  participant R as 整改责任人任一

  C->>H5: 新增默认异常 编辑空白隐患行并可再从隐患库添加
  C->>H5: 填写整改责任人可多人 后提交
  H5->>BE: 提交检查记录(异常)
  BE->>DB: 写入检查记录
  BE->>DB: 自动创建整改通知(待整改)
  BE-->>H5: 返回提交成功(已建单)
  H5-->>C: 提示已自动生成整改通知
  Note over BE: 触发整改流程 多人整改责任人为或签 由平台支撑

  R->>H5: 打开整改通知并填写整改说明/照片
  H5->>BE: 保存隐患整改数据
  BE->>DB: 更新隐患整改信息
  Note over R,DB: 或签任一人填写并通过即可完结 全部隐患完成后进入待复核/已复核
```
...
**一级功能概述**：检查人录入检查结果（新增默认异常）；自动带出可编辑空白隐患行并可继续从隐患库添加；提交后自动建整改通知并触发整改流程；整改责任人多人时或签完结，最终进入待复核/已复核。

#### 3.1.1 检查记录【P0】

| 维度 | 说明 |
|------|------|
| **功能介绍** | 记录安环检查结果；新增默认「异常」；异常时自动带出 1 条可编辑空白隐患行，仍可从隐患库单选继续添加；异常提交后自动创建整改通知并触发整改流程 |
| **前置条件** | 用户已通过企微身份进入应用；检查类型字典、组织人员数据可用 |
| **数据权限** | 检查人查看与本人相关记录；安全管理员可查看全量（具体行权由平台控制，本模块不设计权限配置页） |
...
| 分类 | 内容 |
|------|------|
| **基础字段** | 检查类型、检查日期、检查人（自动填充）、被检查单位（**不含**「责任人」基础字段） |
| **默认结果** | 新增表单检查结果默认选中「异常」；可切换为「正常」 |
| **正常分支** | 说明、上传资料/现场照片 |
| **异常分支** | **整改责任人**（可多人）、整改截止日期、风险等级、风险隐患项目（可多条） |
| **空白隐患行（P0）** | 选中/默认「异常」时，系统自动带出 **1 条可编辑空白隐患行**（空白表单，字段可直接填写）；不自动占用隐患库条目 |
| **添加隐患（P0）** | 自动带出空白行后，仍可点「+ 添加」打开隐患库面板继续追加；分类筛选、查询/重置；展示顺序：隐患类型 → 是否重大隐患 → 隐患描述 → 整改要求；**仅单选**；未选确定提示选择；已添加禁止重复；确定后写入列表并标注来源「隐患库」 |
| **列表列** | 检查编号、检查类型、检查日期、检查人、被检查单位、检查结果、隐患条数、操作（查看/编辑/删除） |
| **底部操作** | 固定「新增检查记录」/「提交」 |
| **提交后【P0】** | 异常提交成功 → 自动建整改通知 → **触发整改流程**（办理页由平台支撑）；多名整改责任人为**或签**（任一人填写并通过即可完结） |

#### 3.1.2 整改通知【P0】

| 维度 | 说明 |
|------|------|
| **功能介绍** | 异常检查提交后由系统自动创建并触发整改流程；整改责任人或签填写整改数据，推进待整改 → 待复核 → 已复核 |
| **前置条件** | 已存在含隐患的异常检查提交记录，或门户处理入口带入对应隐患 |
| **数据权限（业务约定，不画权限页）** | 待整改/待复核：整改责任人可见与本人相关通知，检查人/安全管理员可查看相关或全量。**已复核**：检查人、具有安全管理员职责的业务角色可查看已复核整改数据；具体账号/菜单/数据范围在**平台权限配置**，本 PRD 仅约定业务可见规则，**原型不绘制权限配置页** |
...
| **列表列** | 标题、截止日期、整改责任人、隐患条数、状态 |
| **列表操作 · 查看【P0】** | 各状态均保留「查看」入口。强化说明：①查看为既有能力，须在列表与详情持续可用；②进入只读详情后可阅读关联检查信息、隐患项及已填整改内容（含整改说明/照片）；③**已复核**仍须提供「查看」，用于查阅闭环结果；④查看不替代「整改」操作 |
| **列表操作 · 整改** | 仅待整改等可办理状态展示「整改」；已复核不展示 |
| **列表操作 · 删除** | 按业务规则可用时展示 |
| **详情 · 查看【P0】** | 关联检查信息 + 隐患列表；已有整改填写内容一并只读展示；已复核详情须完整展示闭环结果，避免「只能看到状态、看不到内容」 |
| **整改填写** | 整改说明、整改照片/附件；**不提供「发起审批」按钮**（流转由平台支撑） |
| **或签规则【P0】** | 整改责任人多人为**或签**：任一人完成填写且流程通过即可完结，不要求全员会签 |
...
│     │     ├── 默认「异常」→ 自动带出 1 条可编辑空白隐患行
│     │     │     ├── 可填空白行；可再「添加」→ 隐患库面板（单选）→ 确定写入
│     │     │     ├── 填写整改责任人（可多人）/截止日期/风险等级
│     │     │     └── 提交 → 自动建整改通知并触发整改流程（或签）→ 可进整改通知列表
│     │     └── 可切换「正常」→ 说明/照片 → 提交 → 回列表
│     └── 查看/编辑/删除
│
└── ▶ 整改通知列表（wecom_安环检查_整改通知_list.html）
      ├── Tab：全部/待整改/待复核/已复核
      ├── 「查看」（各状态均可用，含已复核）→ 详情只读（含已填整改内容）
      ├── 「整改」（待整改）→ 整改页 → 隐患编辑页 → 填写说明/照片 → 保存（或签任一人即可）
      └── 「删除」→ 确认删除
```
...
|----------|----------|----------|--------|----------|
| 安环检查 | 检查记录 | 新增默认异常；自动空白隐患行；可再隐患库单选添加；去掉基础责任人，保留整改责任人；异常提交自动建单并触发整改流程 | P0 | 是 |
| 安环检查 | 整改通知 | 自动建单；或签填写；列表/详情「查看」强化（含已复核）；状态待整改/待复核/已复核 | P0 | 是 |
...
- 隐患库单选添加隐患；新增异常默认空行；整改自动建单、或签填写与列表/详情查看

### 8.2 不包含

- 独立 App 与自定义底部 Tab
- 消息中心、审批办理页、角色权限配置页及对应按钮/弹窗（含「谁可看已复核」的权限配置 UI，仅 PRD 业务约定）
...
| 风险 | 说明 | 应对措施 | 优先级 |
|------|------|----------|--------|
| 审核依赖平台 | 建单后的审核、退回、通过、或签办理由平台支撑 | 业务只展示状态与结果；不产出审批 UI | P0 |
| 消息依赖平台 | 待办与提醒由消息中心处理 | 原型与 PRD 不设计消息页 | P0 |
| 或签理解偏差 | 易被实现成全员会签 | PRD 明确「任一人填写并通过即完结」；联调对照平台或签配置 | P0 |
| 已复核可见范围 | 业务约定与平台实际权限配置不一致 | 实施时按本 PRD 业务约定在平台配置；产品不画权限页 | P1 |
| 隐患库初期为空 | 单选无数据时依赖空白行 | 默认空白行可编辑兜底；空状态提示；是否预置条目见【待确认】 | P1 |
...
1. 隐患库初始数据来源——是否需要预置标准隐患条目？空白行与隐患库添加并存后，是否仍需「纯手动录入」第三方式入口？
...
6. 整改责任人「多人」UI：人选控件形态（多选人员）是否沿用平台通讯录组件？或签通过后其余责任人待办如何收敛（平台行为，业务是否需状态文案）？
