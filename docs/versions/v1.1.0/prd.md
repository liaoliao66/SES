...
version: v1.1.0
updated: 2026-07-15
iteration: 按终端分章；PC监督统计+门户重要通知（Banner）与公告后台；企微违章表单；企微推送仅约定不画消息中心
---
...
| **版本号** | v1.1.0 |
| **更新日期** | 2026-07-15 |
| **迭代说明** | ① **PC端**：使用监督统计；安环门户首页**重要通知**强化提醒（Banner 滚动通告条）+ **消息公告后台**（发布/下架）。② **企微H5端**：违章登记/处理表单补强；公告**不在企微端做页面**，发布后由**企微消息推送**提醒查看（推送能力平台支撑，本产品不画消息中心）。③ 按终端分章编写。 |
...
| 终端 | 本版增量 |
|------|----------|
| **PC端** | ① 使用监督统计；② 安环门户首页重要通知（Banner）+ 消息公告管理后台 |
...
| 角色 | 终端 | 职责（业务侧） |
|------|------|----------------|
| 安环负责人 / 管理者 | PC | 查看使用监督统计、导出；发布/管理消息公告 |
| 系统管理员 | PC | 查看全量统计；可管理公告（与管理员规则一致） |
| 门户浏览人 / 全员 | PC | 安环门户查看重要通知与公告详情（**默认全员可见，不做按角色过滤**） |
...
> 用户明细、详情兜底页等子页可不挂侧栏；门户「更多」进入的只读列表仍挂同一固定侧栏。
...
| 序号 | 页面文件名 | 页面名称 | 上游 | 下游 | 状态 |
|------|-----------|----------|------|------|------|
| 8 | `违章管理_违章登记_list.html` | 违章登记列表 | 版本入口 | 新增表单 | 已交付（迁入） |
| 9 | `wecom_违章管理_违章登记_form.html` | 新增违章登记 | 列表 | 回列表 | 已交付（迁入） |
| 10 | `违章管理_违章处理_list.html` | 违章处理列表 | 版本入口 | 处理表单 | 已交付（迁入） |
| 11 | `wecom_违章管理_违章处理_form.html` | 违章处理 | 列表 | 回列表 | 已交付（迁入） |
| 12 | `prototype/versions/v1.1.0/index.html` | 本版本原型入口 | 全局总入口 | 上述全部 | 已交付 |
...
| 本版 PC 增量 | 使用监督统计；**门户 Banner 重要通知 + 消息公告后台** |
...
**PC · 重要通知 / 公告后台：**  
6. 固定侧栏为「安环门户 / 使用监督 / 公告信息→重要通知」，跨页样式一致。  
...
| 后台菜单挂载位置 | 安环域菜单结构 | **固定：安环门户 / 使用监督 / 公告信息→重要通知** |
