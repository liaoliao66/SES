...
version: v1.1.0
updated: 2026-07-15
iteration: 按终端分章；PC监督统计+门户重要通知与公告后台；PC应急联系人维护；企微违章表单；企微推送仅约定不画消息中心
---
...
| **版本号** | v1.1.0 |
| **更新日期** | 2026-07-15 |
| **迭代说明** | ① **PC端**：使用监督统计；安环门户首页**重要通知**（Banner 通告条）+ 公告后台；**应急管理→应急联系人**（名册维护）。② **企微H5端**：违章登记/处理表单补强；公告不在企微端做页面，发布后由企微消息推送（平台支撑，不画消息中心）。③ 按终端分章编写。 |
...
| 终端 | 本版增量 |
|------|----------|
| **PC端** | ① 使用监督统计；② 安环门户重要通知（Banner）+ 公告后台；③ **应急管理→应急联系人**名册维护 |
| **企微H5端** | 违章登记、违章处理页面与字段补强；**不提供**门户公告页（仅接收企微消息推送入口） |
...
| 角色 | 终端 | 职责（业务侧） |
|------|------|----------------|
| 安环负责人 / 管理者 | PC | 查看使用监督统计、导出；发布/管理重要通知；**维护应急联系人** |
| 系统管理员 | PC | 查看全量统计；可管理公告与应急联系人（与管理员规则一致） |
| 门户浏览人 / 全员 | PC | 查看重要通知；**只读查看应急联系人名册**（不可增删改） |
| 检查人 / 登记人 | 企微H5 | 新增违章登记；可收到企微推送后打开链接查看公告（链接落地见【待确认】） |
| 违章处理责任人 | 企微H5 | 填写违章处理并提交 |
| 普通业务用户 | PC/采集端 | 产生有效操作与单据；默认不使用统计菜单 |

> 审批办理页、**平台消息中心列表页**、角色权限配置页由平台统一支撑，本 PRD **不设计**。企微「收到一条推送消息」由企微+平台触达，业务侧仅约定**触发时机与文案要素**。
...
| 企微 H5 违章登记/处理列表+表单及原型 | 按部门/角色过滤公告可见性（本版明确不做） |
| 违章表单字段对齐业务系统 | 系统设置-基础配置使用统计 |
| Excel 导出（统计） | 审批办理 UI、权限配置 UI |
...
| 审批办理 / 权限配置 | 不设计页面 |
| **消息中心列表** | 不设计页面 |
| **企微消息推送** | 公告「发布/置顶重要」后由平台调用企微能力推送；本产品只约定触发条件与推送文案字段，**不画推送配置后台** |
...
| **数据形态** | **一机构多行**：同一机构名称可对应多条人员行（合并单元格仅作展示，存储以行记录为准） |
...
原型/验收用演示数据对齐业务表可见内容；**人员姓名列可全部为空**；序号缺 **5**、部分电话截断或空白 **按图保留，不强行补全**。

| 机构排序 | 机构名称 | 说明 |
|----------|----------|------|
| 1 | 应急指挥部 | 总指挥、副总指挥（副总经理）、副总指挥（总工程师）等 |
| 2 | 应急指挥办公室 | 应急办负责人、联络员 |
| 3 | 通讯联络组 | 组长、组员 |
| 4 | 警戒疏散组 | 组长、组员 |
| 6 | 抢险救援组 | 组长 + 多名组员（业务表缺序号 5） |
| 7 | 后勤保障组 | 组长、组员 |
| 8 | 医疗救护组 | 组长、组员 |
...
| 序号 | 页面文件名 | 页面名称 | 上游 | 下游 | 状态 |
|------|-----------|----------|------|------|------|
| 9 | `违章管理_违章登记_list.html` | 违章登记列表 | 版本入口 | 新增表单 | 已交付（迁入） |
| 10 | `wecom_违章管理_违章登记_form.html` | 新增违章登记 | 列表 | 回列表 | 已交付（迁入） |
...
| 准确 | 排重、排除账号、闭环与业务库一致；过期公告不得展示 | P0 |
...
| 企微H5 | 违章管理 | 违章登记列表/表单、处理列表/表单 | P0 | 是 |
...
| 本版 PC 增量 | 使用监督统计；门户重要通知 + 公告后台；**应急管理→应急联系人** |
| 本版企微增量 | 违章登记/处理完整表单与列表（自 v1.0.0 迁入）；公告仅推送触达 |
...
**PC · 重要通知 / 公告后台：**  
6. 固定侧栏含「安环门户 / 使用监督 / 公告信息→重要通知 / 应急管理→应急联系人」，跨页样式一致。  
...
**企微H5 · 违章：**  
15. 违章登记/处理表单与列表可用；无「发起审核」按钮。  
16. 企微侧无独立公告首页、无应急联系人名册（本版）。
---
...
| 后台菜单挂载位置 | 安环域菜单结构 | **已确认：安环门户 / 使用监督 / 公告信息→重要通知 / 应急管理→应急联系人** |
